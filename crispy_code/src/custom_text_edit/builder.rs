...
///
/// ```
/// # nih_plug_egui::egui::__run_test_ui(|ui| {
/// # let mut my_string = String::new();
/// let response = ui.add(nih_plug_egui::egui::TextEdit::singleline(&mut my_string));
/// if response.changed() {
///     // …
/// }
/// if response.lost_focus() && ui.input(|i| i.key_pressed(nih_plug_egui::egui::Key::Enter)) {
///     // …
/// }
/// # });
/// ```
...
/// # nih_plug_egui::egui::__run_test_ui(|ui| {
/// # let mut my_string = String::new();
/// ui.add_sized(ui.available_size(), nih_plug_egui::egui::TextEdit::multiline(&mut my_string));
/// # });
/// ```
///
///
...
///
/// ```
/// fn selectable_text(ui: &mut nih_plug_egui::egui::Ui, mut text: &str) {
///     ui.add(nih_plug_egui::egui::TextEdit::multiline(&mut text));
/// }
/// ```
///
/// ## Advanced usage
...
impl<'t> TextEdit<'t> {
    ...
    /// let text_edit = nih_plug_egui::egui::TextEdit::multiline(&mut my_string)
    ...
    /// # fn my_memoized_highlighter(s: &str) -> nih_plug_egui::egui::text::LayoutJob { Default::default() }
    /// let mut layouter = |ui: &nih_plug_egui::egui::Ui, string: &str, wrap_width: f32| {
    ///     let mut layout_job: nih_plug_egui::egui::text::LayoutJob = my_memoized_highlighter(string);
    ///     layout_job.wrap.max_width = wrap_width;
    ///     ui.fonts(|f| f.layout_job(layout_job))
    /// };
    /// ui.add(nih_plug_egui::egui::TextEdit::multiline(&mut my_code).layouter(&mut layouter));
    /// # });
    /// ```
    #[inline]
...
impl TextEdit<'_> {
    ...
    /// # nih_plug_egui::egui::__run_test_ui(|ui| {
    /// # let mut my_string = String::new();
    /// let output = nih_plug_egui::egui::TextEdit::singleline(&mut my_string).show(ui);
    /// if let Some(text_cursor_range) = output.cursor_range {
    ///     use nih_plug_egui::egui::TextBuffer as _;
    ///     let selected_chars = text_cursor_range.as_sorted_char_range();
    ///     let selected_text = &my_string[selected_chars];
            ...
            if has_focus {
                if let Some(cursor_range) = state.cursor.range(&galley) {
                    ...
                    if text.is_mutable() && interactive {
                        ...
                        ui.ctx().output_mut(|o| {
                            o.ime = Some(nih_plug_egui::egui::output::IMEOutput {
                                rect: to_global * rect,
